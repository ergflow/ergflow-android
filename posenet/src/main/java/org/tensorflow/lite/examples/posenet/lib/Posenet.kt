...
class Person(keyPoints: List<KeyPoint>, val score: Float) {
    private val keyPointMap: Map<BodyPart, KeyPoint> = keyPoints.map { it.bodyPart to it }.toMap()
    val keyPoints: List<KeyPoint> = keyPointMap.values.toList()
    fun getPoint(bodyPart: BodyPart): KeyPoint? {
        return keyPointMap[bodyPart]
    }
}
